...
#            self.X_test, self.y_test = self.problem.get_test_data(path=ramp_data_dir)
    ...
    def run_next_experiment(self, module_path, fold_i, save_output=False):
        ...
        X_train, y_train, X_test, y_test = rw.utils.assert_data(
            ramp_kit_dir=self.ramp_kit_dir, ramp_data_dir=self.ramp_data_dir,
            data_label=self.data_label, submission_path=module_path)
        cv = rw.utils.assert_cv(
            ramp_kit_dir=self.ramp_kit_dir, ramp_data_dir=self.ramp_data_dir,
            data_label=self.data_label, fold_idxs=self.fold_idxs,
