...
class HyperparameterOptimization(object):
    ...
    def __init__(
        ...
        self.hyperopt_output_path = os.path.join(self.submission_dir, "hyperopt_output")
...
def objective(config, run_params=None):
    ...
    output_submission_dir = f"{hyperparam_opt.submission_dir}_hyperopt_{hyper_hash}"
...
def init_hyperopt(
    ...
    hyperparameters = parse_all_hyperparameters(submission_dir, problem.workflow)
    if engine_name == "random":
        ...
        evaluated_rewards = None
        if resume:
            previous_trial_paths = glob.glob(
                f"{ramp_submission_dir}/{submission}_hyperopt_*"
            )
            ...
            for prev_trial_path in previous_trial_paths:
                ...
                for fold_idx in fold_idxs:
                    try:
                        score = rw.utils.load_submission_fold_score(
                            Path(prev_trial_path),
                            fold_idx,
                            problem.score_types[0].name,
                            "valid",
                            data_label,
                        )
                        scores.append(score)
                ...
                trial_hypers = {h.name: h.default_index for h in hyperparameters_trial}
